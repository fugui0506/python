...
class Config:
    def __init__(self, **kwargs):
        ...
        self.build: list[str] = kwargs.get("build", ["test"])
...
async def run_tasks(path: Path):
    ...
    if not works:
        ...
        print(f'一共 {len(list(works))} 个任务需要处理')
...
async def build(path: Path, index: int):
    ...
    for env in envs:
        ...
        shell("flutter build ios -v --release --no-codesign %s" % (environment), cwd=project_path)
        ...
        new_ipa_path = path / f'{app_name}_{env}_{config.target.version}.ipa'
        old_ipa_path.rename(new_ipa_path)

        shell("flutter build apk -v --release %s"  % (environment), cwd=project_path)
        old_apk_path =  "app/outputs/flutter-apk/app-release.apk"
        new_apk_path = path / f'{app_name}_{env}_{config.target.version}.apk'
...
def set_target(path: Path, config: BuildConfig):
    ...
    print('目标参数设置成功... %s' % config.target.to_json())
...
def update_assets(path: Path, config: BuildConfig):
    ...
    if dst_path.exists() and stc_path.exists():
        print('资源文件夹已存在，正在删除...')
...
def update_yaml(path: Path, config: BuildConfig):
    ...
    print(f"已更新 {pubspec_path} 文件中的 name 和 version")

def update_android(path: Path, config: BuildConfig):
    """更新 Android 文件"""
    print('正在更新 Android 文件...')
    android_path = path / str(config.target.name) / "android/app"
...
def update_dart(path: Path, config: BuildConfig):
    ...
    for dart_file in path.rglob('*.dart'):
        ...
        try:
            ...
            # 替换包名
            if config.target.package and config.project.package:
                new_content = new_content.replace(config.project.package, config.target.package)

            # 替换标题
            if config.target.title and config.project.title:
                new_content = new_content.replace(config.project.title, config.target.title)

            # 替换 Flutter 名称
            if config.target.flutter_name and config.project.flutter_name:
                new_content = new_content.replace(config.project.flutter_name, config.target.flutter_name)

            # 写入修改后的内容
            with dart_file.open('w', encoding='utf-8') as file:
                file.write(new_content)

            print(f'Dart 文件修改成功 --> {dart_file.resolve()}')
        except Exception as e:
            print(f'修改 Dart 文件失败: {dart_file.resolve()}，错误: {e}')

def update_ios(path: Path, config: BuildConfig):
    print('正在更新 IOS 文件...')

    ios_path = path / str(config.target.name) / "ios"
