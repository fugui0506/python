...
SHOREBIRD_ID: str = 'd5da101c-96f0-41fb-9383-6ffff89d205b'

BUILD_ENVS: list[str] = ['test', 'rel', 'pre', 'grey']

# 项目文件夹的名称 [MAIN_PATH = Path.cwd()]
...
def update_yaml(path: Path, config: Any):
    print('正在更新 ShoreBird 文件...')
    shordbird_path = path / 'shorebird.yaml'

    with open(shordbird_path, 'r', encoding='utf-8') as file:
        content = file.read()

    new_content = content
    new_content = re.sub(r'app_id: \s*\S+', f'app_id: {SHOREBIRD_ID}', new_content)
    
    with open(shordbird_path, 'w', encoding='utf-8') as file:
    ...
    if config['name'] == "" and config['version'] == "":
        print("name 和 version 为空，不需要替换")
        return
    ...
    if config['version'] != "":
        new_content = re.sub(r'version: \d+\.\d+\.\d+', f'version: {config['version']}', content)
    
    if config['name'] != "":
        name_without_quotes = str(config['name']).strip('"').strip("'")
...
def build(work_path: str, envs: list[str]):
    ...
    for env in envs:
        ...
        run_command("yes | shorebird release ios --no-codesign %s" % (environment), cwd=work_path)
        ...
        run_command("yes | shorebird release android --artifact apk %s"  % (environment), cwd=work_path)
