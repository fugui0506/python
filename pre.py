...
def create_xcode_archive():
    ...
    new_ipa_path = os.path.join(OUTPUT_DIR, 'cgwallet_%s_%s.ipa' % (NAME.pre.value, BUILD_TIME))
...
def copy_apk_to_output():
    ...
    new_apk_path = os.path.join(OUTPUT_DIR, 'cgwallet_%s_%s.apk' % (NAME.pre.value, BUILD_TIME))
